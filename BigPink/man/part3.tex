...
最初に述べた通り, 反駁エンジンは等号を含む一階述語論理系の上での定理証明
機能を提供するものであるが, これをもう少し具体的に言うと次のようになる：
\begin{itemize}
\item CafeOBJ モジュールで宣言される公理を, 等号を含む一階述語論理(以降
  \textbf{FOPL} と呼ぶ)の論理式とみなし,  
\item そのモジュールの文脈において, 与えられた述語が, モジュールの公理か
  らの論理的帰結となるかどうかを反駁法によって証明する.
\end{itemize}
というものである.
反駁には resolution 原理を用いるが, 等号を扱うための paramodulation 機構
が推論ルールとして含まれている. 推論ルールには, 一般的な binary
resolution に加えて, (negativ/positive) hyper resolution, また unit
resulting resolution が用意されている. また, 論理式に含まれる関数適用フォー
ムの簡単化機構としてdemodulation が組み込まれている. demodulation は関数
フォームだけでなく論理式の書き換えにも用いることが可能となっている. 
...
\subsection{FOPL文}
PigNose における FOPL 文は, 通常の項と同様に CafeOBJ 項である. これは組
み込みモジュール FOPL-CLAUSE(第\ref{sec:builtin-fopl-clause}章を参照)
で定義されている(FOPL文の構文の詳細は第\ref{sec:fopl-syntax}章で説明す
る)が, これは従来の CafeOBJ 言語による記述と出来るだけ境界なしに FOPL 文
を使えるようにするための方策である.  
この時, まず最初に大前提として,
\begin{Verbatim}[frame=single]
      CafeOBJ 組み込みの真偽値のためのソート Bool を, 
      FOPL 真偽値を表現するためのソートとして使用する
\end{Verbatim}
という規定が設けられている. 
従って, 
\begin{center}
\fbox{\textbf{Bool 値の CafeOBJ 項は FOPL の文(述語)と解釈される}}.
\end{center}
その他の CafeOBJ 項は述語の引数として出現するのでない限り, 正当な式とは
見なされない.
CafeOBJ モジュールで記述された公理は, これを基本として以下で述べるように
解釈される.
...
\begin{simplev}
        ＜条件＞ -> ＜左辺＞ = ＜右辺＞ 
\end{simplev}
と解釈する. CafeOBJ の条件部は Bool 値の項であるので, 上のような解釈は
自然にである. また, これは下と等価である.
\begin{simplev}
        ~＜条件＞ | ＜左辺＞ = ＜右辺＞
\end{simplev}
ここで, CafeOBJ 等式 の \verb|=| を FOPL の等号 \verb|=| と解釈している
事に注意. resolution を行う際には, 最後に得られた形の式を, 節形式に変換
して用いる. これはシステムが自動的に行うので, 利用者はこれに関して特に気
...
先に CafeOBJ の Bool 値の項は, 自動的に FOPL 文とみなされると述べた. 
CafeOBJ には Bool を値のソートとするオペレータがいくつか組み込まれている
. これらは組み込みモジュール BOOL とそのサブモジュールで宣言されて
いるものであるが, 表\ref{tab:builtin-pred}に示したように, 対応する
FOPL 論理結合子へとシステムが変換する.  
...
また, 組み込みの xor(exclusive or) に関しては,
\begin{simplev}
    p xor q --> (~p | ~q)&(p | q)
\end{simplev}
のような変換を行う.
...
なお, 具体的な FOPL の文法については, 第\ref{sec:fopl-syntax}章で説明
されている. 

\subsection{制約事項}
CafeOBJ で記述出来る公理は条件付き等式ばかりではなく, 遷移規則も可能であ
るが, これは FOPL 文として扱うことは不可能である. 従って PigNose では,
遷移規則による公理は無視される.
...
\item 組み込みの等式(Lisp関数で定義されているもの)は resolution に
  ...
  但し，demodulator としては使用できるので, システムは自動的に
  これらを demodulator として使用する. 
\end{enumerate}

\subsection{その他の CafeOBJ 組み込みモジュールの扱い}
BOOL モジュールの扱いについては第 \ref{sec:bool-mod} 節で述べたが,
ここでは, NAT や STRING 等のその他の組み込みモジュールの一般的な扱いにつ
...
\end{simplev}
これらのモジュールのうち, PROPC を例外として他の全ての組み込みモジュール
に含まれる公理は次のような取り扱いを受ける：
\begin{itemize}
\item 全ての公理は(組み込みの)demodulator としてのみ使われる.
\item 推論ルールを用いた新たな節の導出には用いられない.
\item 条件付きの等式は無視される.
\end{itemize}
...
が, \verb|FOPL-CLAUSE| という名前の新規の組込見モジュールを輸入している
事(サブモジュールの一つになっている事)が必要である. 
このモジュールは prelude ファイル fopl.mod で定義され, FOPL の論理式の構
文が定義されている. また, システムは, FOPL-CLAUSE が輸入されているモジュー
ルでは, 反駁エンジンで導入された機能が使用できるように実行環境を設定する
\footnote{第 \ref{sec:fopl-loading} 節で述べた通り,fopl.mod は, システム
...
モジュール FOPL-CLAUSE を簡便に輸入するために, 新規の CafeOBJ スイッチ
\texttt{iclude FOPL-CLAUSE} が用意されている.
\begin{examplev}
  set include FOPL-CLAUSE on   -- 自動輸入を有効にする (初期値)
  set include FOPL-CLAUSE off  -- 自動輸入を無効にする
\end{examplev}
このフラグを, 上の例のように \texttt{on} に設定する事により, 
モジュール内で \texttt{ax} あるいは \texttt{goal} による公理宣言
(第\ref{sec:new-construct}章を参照)が出現した場合に, 必要に応じて
FOPL-CLAUSE が自動的に輸入される. 

\section{FOPL の構文}
\label{sec:fopl-syntax}

ここでは \verb|FOPL-CLAUSE| で定義される FOPL の構文を示す．FOPL の文は 
CafeOBJ 項で表現される．そのソートは \verb|FoplSentence| と名付けられ，
下の CafeOBJ ソート宣言文に示されているように, 組み込みソート Bool の上
位ソートとして宣言されている.  
\begin{description}
   \item \textbf{FOPL 文のソート}
     \begin{simplev}
       [Bool < FoplSentence]
     \end{simplev}
\end{description}
...
次に，限定子に関しては下のように定義されている．
\begin{description}
  \item \textbf{全称限定子(forall)}
  \begin{simplev}
op \A[_] _ : VarDeclList FoplSentence -> FoplSentence {prec: 125}
  \end{simplev}

  \item \textbf{存在限定子(exists)}
  \begin{simplev}
...
\subsection{述語・命題の宣言}
先に述べた通り, ソート \verb|FoplSentence| の項がFOPL 文であり,
Bool 値のオペレータは一般に述語として解釈されるので,
述語や命題は通常の CafeOBJ のオペレータ宣言によって行うことが
できる. 命題の場合は, 引数を持たない Bool の定数項として宣言すれば良い.

このためには, 下のように CafeOBJ の \verb|pred| 構文を用いるのが
便利である：
\begin{examplev}
...
以下の新規構文が導入された. いずれも FOPL 文を用いた公理の宣言を可能とす
るものである. これらは反駁エンジンの実行の際に, その他の CafeOBJ の通常
の公理と併せて推論に用いられる. ただし, CafeOBJ の簡約コマンド
...
\end{simplev}
    ...
    \verb|<項>| は，ソート FoplSentence の項でなければならない．
...
. これらは先に述べた通り, 全称限定子によって暗黙的に束縛される.
従って例えば
\begin{simplev}
    ax 0 <= M & 0 <= N -> 0 <= M + N .
\end{simplev}
...
\begin{simplev}
    option { reset | = <オプション名> }
\end{simplev}
  \verb|<オプション名>| は, 以前に save-option コマンドを用いて保存して
  おいたオプションの名前を指定する.
  \verb|reset| を指定すると，全ての値が初期値に再設定される．
...
\end{simplev}
 ...
 {\small
  ...
  \textbf{注意}: flag, param, option の3つの場合を除き,
...
\item[clause コマンド]
  指定された項を節形式に変換し, 結果を印字する. 試験のためのコマンドであ
  る.
\begin{simplev}
  <clauseコマンド> ::= clause <項> .
\end{simplev}
...
\item[show/describe コマンド]
  従来の CafeOBJ コマンド \verb|show| と \verb|describe| を拡張したものである.
\begin{simplev}
  <showコマンド>      ::= show <節ID> 
...
\end{simplev}
  ...
  `list axioms' や，`list sos' 等で表示される内容から知ることができる．
   describe の場合は, 指定の節が CafeOBJ モジュールの公理を節形式に
...
\item[(2)] (必要に応じて) \texttt{FOPL-CLAUSE} をインポート\\
  証明実行の文脈となるモジュールのサブモジュールに FOPL-CLAUSE 
...
\begin{simplev}
    <節番号>:[<導出履歴>] FOPL文
\end{simplev}
ここで, $<$節番号$>$ はシステムが節に自動的に割り付ける自然数である.
...
FOPL 文から節形式へ変換される場合, 存在限定子があった場合に
Skolem 関数が自動生成される. この表示の一般形式は次の通りである：
\begin{simplev}
  定数の場合    ： #c-<N>.<ソート>
  引数を持つ場合： #f-<N>.<ソート>
\end{simplev}
ここで, $<$N$>$ は自然数であり, システムが適当に割り付ける.
また $<$ソート$>$ は関数の値ソートの名前である.
...
導出履歴の欄は空(\verb:[]:)となっている. 続いて節を構成する
リテラルが表示されている. この場合は単一節で, リテラルは一つである.
\verb:#c-1.Account: はシステムが生成した Skolem 定数である.
\begin{simplev}
  1:[] 0 <= balance(#c-1.Account)
\end{simplev}
次の例(節126)も単一節の表示例であるが, 導出項であり
導出履歴欄が表示されている.
この例の場合, \verb:para-from: による paramodulation from 推論ルール
に続いて, \verb:unit-del: による unit deletion が施されたことが解る.
...
\item[sos から `節を一つ選' ぶ方法] 利用者がフラグによって設定する．
  ...
  given clause の選択に関係するその他のフラグについては, 第
  ~\ref{sec:loop-flags} 節を参照されたい. 
...
\item [`現在有効な推論ルール群'] 前節で述べた通り，反駁エンジンには
  ...
  上に示したフラグと合わせて, 推論ルールの選択に関係するフラグは, 第
  ~\ref{sec:inf-flags} で説明されている. 
...
このためのコマンドが \verb:save-option: である(第~\ref{sec:new-commands}
章を参照).  \verb:save-option: で名前を付けて保存したオプション(全てのフ
ラグ/パラメータとそれらの値の組み)は \verb:option: コマンドによって再利
用することが出来る. また, オプションを全て初期値にリセットするのにも,
\verb:option: コマンドを使用する.
...
\noindent
現在のフラグの設定値を知るには \verb:list: コマンドを用いる.
\begin{examplev}
  list flag
\end{examplev}

\noindent
また, パラメータの設定値を知るにも, 同じ \verb:list: コマンドを
用いる.
\begin{examplev}
  list param
\end{examplev}
...
\item[prop-res] 初期値は off. on の場合, 以下の条件が満足される
  ...
  \begin{itemize}
  \item フラグ \verb:binary-res: が off
  \item フラグ \verb:hyper-res: あるいは \verb:neg-hyper-res: が on
  \end{itemize}
...
\item[dist-const] 初期値は off. on の場合, 全ての定数は互いに異なるもの
  ...
  $c_1$ と $c_2$ が異なる定数項であった場合, \verb:false: とされ, 同一の
  場合は \verb:true: として扱う. Skolem 定数に関してはこのフラグは無視
  される. 
\end{description}


\subsubsection{Paramodulation に関するフラグ}
...
\item[unit-deletion] 初期値 off. on の場合,
  ...
  例えば \verb:p(a,X) | q(a,X): という節の2番目のリテラルは,
  単一節 \verb:~ q(u,V): によって除去される(ここで, X と V は変数であり,
  他は定数とする--以下同様).
  しかし, 単一節 \verb:~ q(u,b): によっては除去されない. 
...
\item[delete-identical-nested-skolem] 初期値 off. on の場合,
  ...
  例えば, \verb:f: が Skolem 関数であるような場合, \verb:f(f(x)):
  や \verb:f(g(f(x))): というような形の項を含む節を除去する.
...
\item[eq-units-both-ways] 初期値 off. on の場合, 等式のみからなる単一節
  ...
  \verb:order-eq: の値に依存する：
  \begin{itemize}
  \item もし \verb:order-eq: が off ならば, 等式 $\alpha=\beta$
    に関して $\beta=\alpha$ が無条件に自動生成される.
  \item \verb:order-eq: が on ならば, 等式の左右辺の向き付けが
...
\item[dynamic-demod] 初期値 off. on の場合,
  ...
  照. このフラグを on に設定すると, 自動的に \verb:order-eq: も on に設
  定される. 

\item[dynamic-demod-all] 初期値 off. on の場合,
  システムは全ての新たな導出節を demodulator とすべく試みる(第
  ~\ref{sec:dynamic}節を参照). 
  このフラグを on に設定すると, 自動的に \verb:dynamic-demod: と
  \verb:order-eq: が on に設定される.
...
\item[back-demod] 初期値 off. on の場合, 
  ...
  このフラグが on にされると, 自動的に \verb:order-eq: と 
  \verb:dynamic-demod: フラグが on に設定される.
...
\item[lex-order-vars] 初期値 off. このフラグは lex-依存の demodulation
  ...
  フラグ \verb:lrpo: が on になっている場合, lex-order-vars の値は
...
\item[process-input] 初期値 off. on の場合, 初期の usable および
  ...
  に対する処理では, 以下の違いがある：
  \begin{itemize}
  \item[(1)] 入力節に対しては, \verb:max-literals: および 
    \verb:max-weight: パラメータによる制約テストは行われない.
    また, \verb:delete-identical-nested-skolem: フラグによるテストも
    行われない.
...
\item[order-hyper] 初期値 on. on の場合, 推論ルールの \verb:hyper-res:
  と \verb:neg-hyper-res: による導出の際に, 順序付け戦略による
...
\end{examplev}
では, \verb:para-from: と \verb:unit-del: が導出ルール
に相当し, \verb:para-from: は, 節6 と 2 が, また
\verb:unit-del: では, 節番号 1 をオプションの $<$節番号$>$
として持つ. 以下で, 各導出ルールの説明をする.
\begin{description}
\item[bin-res:clause-1,clause-2]
  \verb:bin-res: によって生成された. clause-1 は given clause の
  節番号. clause-2 は, これと resolve した節の節番号.
\item[prop-res:clause-1,clause-2]
  \verb:prop-res: によって生成された. clause-1 は given clause の
  節番号. clause-2 は, これと resolve した節の節番号.
\item[hyper-res:clause-1,claus-2,..,clause-n]
  \verb:hyper-res: によって生成された. clause-1 は given clause の
  節番号. clasuse-2 .. clause-n はこれと resolve した節の節番号.
\item[neg-hyper-res:clause-1,clause-2,...,clause-n]
  \verb:neg-hyper-res: によって生成された. clause-1 は given clause の
  節番号. clause-2 ... clause-n はこれと resolve した節の節番号.
\item[para-into:clause-1,clause-2] \verb:para-into: によって生成された.
  clause-1 がオリジナルな節, clause-2 が paramodulator である.
\item[para-from:clause-1,clause-2] \verb:para-from: によって生成された.
  clause-1 がオリジナルの節, clause-2 が paramodulator である.
\item[fsimp:] factor simplification(verb:factor-simp:)によって
  生成されて.
\item[back-demod:clause-1,claus-2,...,clause-n] clause-1...clause-n によって
  back demodulate されて生成された. この場合, もともとのオリジナルな
  節の内容ば破壊的に変更されている.
\item[demod:clause-1,clause2,...,clause-n] clause-1..clause-n によって
  demodulation が施されて生成された. back-demod の場合と同様,
  オリジナルな節の内容は破壊的に変更されている.
\item[copy:clause-1] 内部処理でclause-1の節をコピーして生成された.
\item[flip:] 等式の左右辺の入れ換えによって生成された.
\item[unit-del:clause-1] clause-1 との \verb:unit-del:(unit deletion) に
  よって生成された.
\item[back-unit-del:clause-1] clause-1 との \verb:back-unit-del:(back unit
  deletion)によって生成された.
\end{description}
...
フラグ \verb:order-eq: が on の場合には等式の左右辺の項の大小比較が行わ
...
Hoc な順序付けとも呼ばれる. ) 
フラグ \verb:lrpo: がこれらどちらの順序つけを使用するかを決め, これが
...
\item[辞書式順]
  ...
  が, 項の大小関係を決定する.
  変数記号との比較はフラグ \verb:lex-order-vars: の値に依存する.
  \begin{description}
  \item[{\tt lex-order-vars} が on.]
    この場合変数も比較の対象となる. 
  ...
  \item[{\tt lex-order-vars} が off (初期値).]
...
\item[重み-辞書式順]
  ...
  大きいと判定される. 両方の項の重みが同じ場合は, \verb:lex-order-vars:
...
フラグ \verb:order-eq: が on で, \verb:lrpo: が off の場合, 導出節に含ま
...
フラグ \verb:dynamic-demod: あるいは, \verb:dynamic-demod-all: が on の
場合, フラグ \verb:order-eq: も自動的に on になっているはずである.  
この場合, システムは推論の過程で導出された正の等式リテラルのみを含む単一
節を, 以降 demodulator(書き換え規則) として用いるべく試みる. 
もし, \verb:process-input: が on であった場合には, usable 及び sos に含
...
\item もし 「重み-辞書式順」 の意味で $\alpha\succ\beta$ であり,
  ...
  \begin{itemize}
  \item[(a)]
    もし \verb:dynamic-demod-all: が on であれば, 等式を
    demodulator とする;
  \item[(b)]
    もし \verb:dynamic-demod-all: が off であり, $wt(\beta) \leq 1$, 
    ならば, 等式を demodulator とする.
  \end{itemize}

\item  もし, フラグ \verb:dynamic-demod-lex-dep: と
  \verb:dynamic-demod-all: の両方が on の場合は, 
  \begin{enumerate}
...
\subsubsection{項の順序付け ({\sc lrpo})}

辞書式再帰パス順序({\em lexicographic recursive path ordering}) は項の比
較を行う方法もう一つの方法であり, フラグ lrpo が on の時に用いられる.
重要な理論的性質として {\sc lrpo} は, {\em termination} 順序であるという
ことがある.  
つまり, $R$ を demodulator の集合とし, 含まれる各 demodulator の左辺は
{\sc lrpo} の意味で右辺より大きいならば, demodulation (demodulator を左
辺から右辺の向きに適用する) プロセスは終了することが保証される.

{\sc lrpo} による比較は, (1) 等式リテラルの方向つけ(左辺が右辺より大きく
なるように置き換える事), (2) 等式が demodulator として使われ得るか, ある
いは {\sc lrpo}-依存の demodulator となるかを判定したり, 
(3) {\sc lrpo}-依存の demodulator を適用できるか否かを判定するのに用いら
れる. 上記以外の文脈で使用されることは無い.

\subsubsection{オペレータ記号の順序付け -- lex コマンド}
\label{sec:lex-command}

{\sc lrpo} を使用する場合, オペレータ記号に関する順序つけが決められてい
なければならない. これは lex コマンドを用いて指定することが出来る.
コマンドの構文は次の通りである：
\begin{simplev}
 <lex コマンド> ::= lex(<op1_1>,....,<op_n>)
\end{simplev}
\verb:<op1_1>: や \verb:<op_n>: はオペレータ記号である. 左側にある程
小さいと規定する. 特殊なオペレータ記号として \verb:*: と 
\verb:SKOLEM: の2つがシステムで予約されている：
\begin{description}
...
\verb|*/2| とすることでこれを参照することが出来る. もしこれが定数であれ
ば \verb|*/0| とすれば良い. SKOLEM の場合も同様である.
...
\subsubsection{等式の方向つけ ({\sc lrpo})} \label{sec:orient-lrpo}

もし, フラグ \verb:order-eq: が on であり, ある等式リテラルの
どちらかの引数が他方より {\sc lrpo} 順で大きいならば, 
大きい方の引数が左辺におかれるように並び替えられる.

\subsubsection{Dynamic Demodulator の判定 ({\sc lrpo})} \label{sec:dynamic-lrpo}

フラグ \verb:dynamic-demod: が on の場合, 
システムは
全ての等式に関して, それらが demodulator として使えないかどうかを
調べる(フラグ \verb:dynamic-demod-all: は, \verb:lrpo: が
on の場合は無視される.)
等式 $\alpha=\beta$ において,
{\sc lrpo} 順で $\alpha\succ\beta$ であれば,
等式は demodulator となる
(この判定が行われる時点では, すでに等式の方向つけが済んでいる). 
もし, \verb:dynamic_demod_lex_dep: が on であり, 
等式のどちらの辺も他の辺に対して {\sc lrpo} の意味で小さくなく,
かつ $\beta$ に含まれている変数は全て $\alpha$ にも出現する
ものであれば, 等式は {\sc lrpo}-依存の demodulator とされる.

\subsubsection{{\sc lrpo}-依存の Demodulation ({\sc lrpo})} 
\label{sec:lex-dep-lrpo}

{\sc lrpo}-依存の demodulator は, 書き換えられた結果の項が
元の項より {\sc lrpo} の意味で小さくなる場合にのみ
適用される.
