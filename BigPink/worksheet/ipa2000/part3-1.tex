...
第III部では「検証推論システム」を構成する3つのサブシステム
「反駁システム」, 「仕様検証システム」および「モデル検査システム」の
仕様方法を述べる. 最初に核となる「反駁システム」について説明し,
次いで「仕様検証システム」, 「モデル検査システム」の順に説明する.

\section{反駁システムの使用手引}

反駁システムは，等号を含む一階述語論理の上での定理証明システムであり，
resolution に基づいた反駁エンジンを提供するものである．
このことから，以下では，反駁システムを「反駁エンジン」あるいは，
単に「エンジン」と呼ぶ事がある．

\subsection{反駁システムの機能}
\label{sec:func-abst}

反駁エンジンは resolution calculus に基づく定理証明エンジンであり, 
CafeOBJ インタプリタに組込みとなっている. 
その基本機能は,
\begin{itemize}
...
以下の新規構文が導入された. いづれも FOPL 文による公理を宣言するための
ものである. これらは反駁エンジンの実行の際に, その他の CafeOBJ の通常の
公理と併せて推論に用いられる. ただし, CafeOBJ の簡約コマンド(\texttt{reduce},
\texttt{breduce}, \texttt{exec}) で用いることは出来ない. 
反駁エンジンによる定理証明にのみ有効である.
...
\begin{figure}[htbp]
  \begin{center}
    ...
    \includegraphics{spec-check-arch.eps}
    \caption{仕様検証システムの構成}
