...
mod! SEM-PGM{
  ...
  op err : -> State .
  trans S end  => S .
  trans S (int A ;)  => if in?(A,S) then S else (A :: na) S fi .
--   ctrans S (int A ;)  => S            if in?(A,S) .
--   ctrans S (int A ;)  =>  (A :: na) S if not in?(A,S) .
  trans S A := E ;  => update(A,S[E],S) .
--  ctrans S A := E ;  => update(A,S[E],S) if in?(A, S) .
   trans S (BP P)  => (S BP) P .

  trans S if(E){P1} => if S[E] == 0 then S else S P1 fi .
--  ctrans S if(E){P1} =>  S P1 if S[E] =/= 0 .
--  ctrans S if(E){P1} =>  S    if S[E] == 0 .
  trans S (while E { P }) => if S[E] == 0 then S else S (P while E { P }) fi .
...
mod! MPGM{
  ...
  op recvany(_); : Var -> BPgm .
  op send'(_,_); : Var Int -> BPgm  .
  op recv'(_,_); : Var Int -> BPgm  .
--  op any : -> Exp .
}
...
mod! SEM-MPGM{
 ...
 vars X1 X2 : Var .
 trans (S1 send(X1, Dest);) P1 => (S1 send'(X1, S1[Dest]);) P1 .
 trans (S1 recv(X1, Dest);) P1 => (S1 recv'(X1, S1[Dest]);) P1 .

 trans (((pid :: I) S1) send'(X1, J);) P1 | (((pid :: J) S2) recv'(X2, I);) P2  => 
 (((pid :: I) S1) P1) | (update(X2, ((pid :: I) S1)[X1], ((pid :: J) S2)) P2) .
 trans (S1 send'(X1, J);) P1 | (((pid :: J) S2) recvany(X2);) P2  => 
 (S1 P1) | (update(X2, S1[X1], ((pid :: J) S2)) P2) .
