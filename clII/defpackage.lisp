...
(defmacro DEFPACKAGE (name &rest options)
  ...
  (loop:loop for (option) in options
  ...
  (labels ((option-test (arg1 arg2) (when (consp arg2) (equal (car arg2) arg1)))
	   (option-values-list (option options)
	     (loop:loop for result first (member option options ':test #'option-test)
    ...
    (loop:loop for option in '(:size :documentation)
    ...
    (let ((nicknames (mapcar #'string (option-values ':nicknames options)))
	  ...
	  (shadowing-imported-from-symbol-names-list (loop:loop for list in (option-values-list ':shadowing-import-from options)
							   collect (cons (string (first list)) (mapcar #'string (rest list)))))
	  (imported-from-symbol-names-list (loop:loop for list in (option-values-list ':import-from options)
						 collect (cons (string (first list)) (mapcar #'string (rest list)))))
	  (exported-from-package-names (mapcar #'string (option-values ':export-from options))))
        (flet ((find-duplicates (&rest lists)
		 (let (results)
		   ...
		   results)))
	  (loop:loop for duplicate in (find-duplicates shadowed-symbol-names interned-symbol-names
						  (loop:loop for list in shadowing-imported-from-symbol-names-list append (rest list))
						  (loop:loop for list in imported-from-symbol-names-list append (rest list)))
		do
	    (error "The symbol ~s cannot coexist in these lists:~{ ~s~}" (first duplicate)
		   (loop:loop for num in (rest duplicate)
			 collect (case num (1 ':SHADOW)(2 ':INTERN)(3 ':SHADOWING-IMPORT-FROM)(4 ':IMPORT-FROM)))))
	  (loop:loop for duplicate in (find-duplicates exported-symbol-names interned-symbol-names)
		do
	    (error "The symbol ~s cannot coexist in these lists:~{ ~s~}" (first duplicate)
		   (loop:loop for num in (rest duplicate) collect (case num (1 ':EXPORT)(2 ':INTERN))))))
