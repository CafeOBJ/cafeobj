...
;;; $Id: cexec.lisp,v 1.21 2009-02-23 09:24:34 sawada Exp $
(in-package :chaos)
#|=============================================================================
			     System:CHAOS
			    Module:thstuff
	...
	(do* ((rls rules (cdr rls))
	  ...
	  (let* ((patterns nil)
		...
		(if (method-is-commutative head)
		    (let ((ac-ext (!axiom-ac-extension rule)))
		      (unless ac-ext
			(setq ac-ext (compute-AC-extension rule head)))
		      (when ac-ext
			(push (car ac-ext) patterns)))
		  ;;
		  (let ((a-exts (!axiom-A-extensions rule)))
		    (unless a-exts
		      (setq a-exts
			(compute-A-extensions rule head)))
		    (dolist (r a-exts)
		...
		(when (cexec-pat-match (axiom-lhs pat) target)
		  (push (make-rule-pat pos pat) res))
		||#
		...
		(multiple-value-bind (gs sub no-match eeq)
		  ...
		  (push (make-rule-pat pos pat sub) res)
		  (loop
...
;;;
(defun apply-rule-cexec (rule target subst)
  ;; (apply-rule rule target)
...
(defun term-pattern-included-in-cexec (t1 t2 max-depth &optional cond)
  (multiple-value-bind (max sym)
	...
	("*" (setq zero? t))
	(otherwise nil))
      (rwl-search* t1 t2 most-positive-fixnum max zero? final? cond nil *current-module*)
      )))
