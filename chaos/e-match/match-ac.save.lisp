...
;;; $Id: match-ac.save.lisp,v 1.2 2007-04-03 06:46:14 sawada Exp $
(in-package :chaos)
#|==============================================================================
				  System:Chaos
				 Module:e-match
...
;;; Modified to fit to Chaos by T. Sawada.
...
;;; type : fixnum
(defmacro AC-state-lhs-mask (____state)  `(the fixnum (%svref ,____state 9)))

;;; funs accounted for by RHS-c-sol
;;; type : fixnum
(defmacro AC-state-lhs-f-mask (____state)  `(the fixnum (%svref ,____state 10)))

;;; bit vector of all repeated (> 0) terms on LHS
(defmacro AC-state-lhs-r-mask (____state)  `(the fixnum (%svref ,____state 11)))
...
;;; type : fixnum
(defmacro AC-state-rhs-c-max (____state)  `(the fixnum (%svref ,____state 13)))
...
;;; type : fixnum
(defmacro AC-state-rhs-f-max (____state)  `(the fixnum (%svref ,____state 15)))

;;; type : fixnum 11111 ... 1111
(defmacro AC-state-rhs-full-bits (____state)  `(the fixnum (%svref ,____state 16)))
...
;;; type : fixnum
(defmacro AC-state-lhs-c-count (____state)  `(the fixnum (%svref ,____state 19)))

;;; number of functions on LHS after simplification
;;; type : fixnum
(defmacro AC-state-lhs-f-count (____state)  `(the fixnum (%svref ,____state 20)))

;;; number of variables on LHS after simplification
;;; type : fixnum
(defmacro AC-state-lhs-v-count (____state)  `(the fixnum (%svref ,____state 21)))

;;; number of constants on RHS after simplification
;;; type : fixnum
(defmacro AC-state-rhs-c-count (____state)  `(the fixnum (%svref ,____state 22)))

;;; number of functions on RHS after simplification
;;; type : fixnum
(defmacro AC-state-rhs-f-count (____state)  `(the fixnum (%svref ,____state 23)))
...
(defun AC-next-state-sub (state)
  (do* ((m 0)				; only initialize these vars 
    ...
    (declare (type #+GCL vector #-GCL simple-vector rhs-c-compat) 
	     (type fixnum m lhs-r-mask rhs-c-count))
