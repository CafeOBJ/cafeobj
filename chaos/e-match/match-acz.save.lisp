...
;;; $Id: match-acz.save.lisp,v 1.2 2007-04-03 06:46:14 sawada Exp $
(in-package :chaos)
#|==============================================================================
				  System:Chaos
				 Module:e-match
...
;;;
(defmacro make-match-ACZ-state ()  `(alloc-svec 26))
...
;;; type : fixnum
(defmacro match-acz-state-lhs-mask (state__)  `(the fixnum (%svref ,state__ 9)))

;;; funs accounted for by RHS-c-sol
;;; type : fixnum
(defmacro match-acz-state-lhs-f-mask (state__) `(the fixnum (%svref ,state__ 10)))

;;; bit vector of all repeated (> 0) terms on LHS
(defmacro match-acz-state-lhs-r-mask (state__)  `(the fixnum (%svref ,state__ 11)))
...
;;; type : fixnum
(defmacro match-acz-state-rhs-c-max (state__) `(the fixnum (%svref ,state__ 13)))
...
;;; type : fixnum
(defmacro match-acz-state-rhs-f-max (state__) `(the fixnum (%svref ,state__ 15)))

;;; type : fixnum 11111 ... 1111
(defmacro match-acz-state-rhs-full-bits (state__) `(the fixnum (%svref ,state__ 16)))
...
;;; type : fixnum
(defmacro match-acz-state-lhs-c-count (state__) `(the fixnum (%svref ,state__ 19)))

;;; number of functions on LHS after simplification
;;; type : fixnum
(defmacro match-acz-state-lhs-f-count (state__) `(the fixnum (%svref ,state__ 20)))

;;; number of variables on LHS after simplification
;;; type : fixnum
(defmacro match-acz-state-lhs-v-count (state__) `(the fixnum (%svref ,state__ 21)))

;;; number of constants on RHS after simplification
;;; type : fixnum
(defmacro match-acz-state-rhs-c-count (state__)  `(the fixnum (%svref ,state__ 22)))

;;; number of functions on RHS after simplification
;;; type : fixnum
(defmacro match-acz-state-rhs-f-count (state__) `(the fixnum (%svref ,state__ 23)))
...
(defun match-ACZ-equal (t1 t2)
  ...
  (if (term-is-applform? t2)
	...
	(if (method-is-of-same-operator op (term-head t2))
	    (let ((sub1 (list-acz-subterms t1 op))
		  (sub2 (list-acz-subterms t2 op)))
	      (declare (type list sub1 sub2))
	      ;; note list-acz-subterms eliminates identity terms.
...
(defun match-ACZ-state-initialize (sys env)
  ...
  (block TOP
	...
	(let* ((lhs-1 (equation-t1 equation))
	       ...
	       (rhs-meth (term-method rhs-1))
	       (lhs-2 (list-ACZ-subterms lhs-1 lh-meth))
	       (rhs-2
		(if (method-is-AC-restriction-of rhs-meth lh-meth)
...
(defun match-ACZ-next-state (state)
  ...
  (if (not (match-ACZ-state-p state))
	  ...
	  (if (trivial-match-ACZ-state-no-more-p state)
		...
		(values (trivial-match-ACZ-state-sys state) state nil)))
				...
				(let ((lhs-v (match-ACZ-state-lhs-v state))
				  ...
				  (dotimes (i (length lhs-v) t)
					...
					(unless (sort<=
					  ...
					  (return nil))))))
			   (let ((sol (match-ACZ-solution-from-state
				       state)))
			     (if sol
				 (return (values sol state nil))
				 (return (match-acz-next-state state))))
			   (progn
			     ;; failed at f-level
			     ;; (deallocate-acz-state state)
