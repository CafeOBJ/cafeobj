...
;;; $Id: bmodule.lisp,v 1.1.1.1 2003-06-19 08:29:30 sawada Exp $
(in-package :chaos)
#|==============================================================================
				 System: Chaos
			       Module: primitives
...
(defmacro parameter-module-theory (_mod_)
  `(third (module-name ,_mod_)))
...
;;; 
#||
(defstruct (module-ex-info
	     ;; #+gcl (:static t)
	     )
  (protected-modules nil)
  (type :user)				; the module type, :user :system, or :hard.
  (kind	:module)			; kind of module, one of
					;  :theory, :object, :module
  ;; (all-submodules-list nil)		; list of all submodules, obsolate
  infos					; various misc infos, a p-list.
  )
||#
...
;;;  error-
(defun module-infos (mod) (module-ex-info mod))
(defsetf module-infos (mod) (values)
  `(setf (module-ex-info ,mod) ,values))

;;; PROTECTED MODULES
(defmacro module-protected-modules (_mod)
  ` (getf (module-ex-info ,_mod) ':protected-modules))

;;; TYPE
(defmacro module-type (_mod)
  `(getf (module-ex-info ,_mod) ':module-type))
...
(defmacro module-kind (_mod)
  `(getf (module-ex-info ,_mod) ':module-kind))
...
(defmacro module-is-loose (_mod_)
  ` (memq (module-kind ,_mod_) '(:theory :module)))
...
;;;   (or (module-ex-info-all-submodules-list (module-ex-info mod))
;;;       (setf (module-ex-info-all-submodules-list (module-ex-info mod))
...
(defun add-imported-module (module mode submodule)
  (declare (type module module submodule)
	   (type symbol mode)
	   (values t))
  ;; (setf (module-ex-info-all-submodules-list (module-ex-info module)) nil)
  (add-depend-relation module mode submodule))
...
(defmacro module-void-sorts (_mod)
  `(getf (module-ex-info ,_mod) ':void-sorts))

;;; VOID-METHODS
(defmacro module-void-methods (_mod)
  `(getf (module-ex-info ,_mod) ':void-methods))

;;; SORTS-FOR-REGULARITY
(defmacro module-sorts-for-regularity (_mod)
  `(getf (module-ex-info ,_mod) ':sorts-for-regularity))

;;; METHODS-FOR-REGULARITY
(defmacro module-methods-for-regularity (_mod)
  `(getf (module-ex-info ,_mod) ':methods-for-regularity))
...
(defmacro module-has-behavioural-axioms (_mod)
  `(getf (module-ex-info ,_mod) ':has-behavioural-axioms))
...
(defmacro module-instance-db (mod)
  `(getf (module-ex-info ,mod) ':instance-db))
...
(defmacro module-protected-mode (mod)
  `(getf (module-ex-info ,mod) ':protected-mode))
...
(defmacro module-beh-stuff (_mod)
  `(getf (module-ex-info ,_mod) ':beh-stuff))

;;; BEH-ATTRIBUTES
;;; **************
(defmacro module-beh-attributes (_mod)
  `(getf (module-ex-info ,_mod) ':beh-attributes))

;;; BEH-METHODS
;;; ************
(defmacro module-beh-methods (_mod)
  `(getf (module-ex-info ,_mod) ':beh-methods))

;;; METHODS-WITH-BEH-AXIOM
;;; **********************
(defmacro module-methods-with-beh-axiom (_mod)
  `(getf (module-ex-info ,_mod) ':methods-with-beh-axiom))

;;; BEH-TO-BE-PROOVED
;;; ******************
(defmacro module-beh-to-be-prooved (_mod)
  `(getf (module-ex-info ,_mod) ':beh-to-be-prooved))
	...
	(let ((subs (module-all-submodules c)))
	  ...
	  (unless tmod
	    ...
	    (with-output-chaos-error ('no-such-module)
	      (format t "no such module ~a in the current context."
		      qname)
	      ))
	  (when (atom tmod) (setq tmod (list tmod)))
	  (when (cdr tmod)
	    (with-output-chaos-error ('ambiguous-module-name)
	      (format t "module name ~a is ambiguous in the current context."
		      qname)
	      ))
	  (setq c (car tmod))))
      (car tmod))))
...
(defun initialize-module (mod)
  ...
  ;; misc infos
  (setf (module-ex-info mod) nil)
  ;;; (if (module-ex-info mod)
