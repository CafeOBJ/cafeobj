...
mod* PERMISSIONS (P :: PROJECT) {
  ...
  *[ Permissions ]*
  ...
  eq Uid plays-role Rid in-team Tid given make-project (Vid, Pid, Pm) = 
		if ((Uid == Vid) and (Rid == "Project Manager") and 
  ...
  eq Tid plays-role "Team Manager" in-project Pid given Pm = false .
}
...
mod* ACCESS-CONTROL (P :: PROJECT) {
  *[ Acc-Sys ]*
  ...
  bop perms_ : Acc-Sys -> Permissions		-- projection
  ...
  eq perms (Uid adds-team-to AccSys) = 
	...
	then add-perm (initial-team-name Uid, Uid, "Team Manager", 
...
open TEST-ACCESS-CONTROL 
  ...
  eq FM1 = ("Framework Manager" adds-team-to init-acc-sys) .
