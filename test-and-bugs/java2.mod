...
module* RULE-BASE {
  imports {
    protecting (PARTIALORDER[ACTION { sort Elt -> Action }]
		* { sort PartialOrderOK<X> -> PartialOrderOK<Action>,
		    sort PartialOrder<X> -> PartialOrder<Action>,
		    sort Set<X> -> Set<Action>,
		    sort Pair<X;X> -> Pair<Action;Action>,
...
module! EVENTSPACE {
  ...
  axioms {
    ...
    eq eventspace(po) = if (po =/= ((bottom):PartialOrderErr<Action>) and
                            rule.RULE1(po) and rule.RULE2(po) and
                            rule.RULE3(po) and rule.RULE4(po) and
                            rule.RULE5(po) and rule.RULE6(po) and
                            rule.RULE7(po) and rule.RULE8(po) and
                            rule.RULE9(po) and rule.RULE10(po) and
                            rule.RULE11(po) and rule.RULE12(po) and
                            rule.RULE13(po) and rule.RULE14(po) and
                            rule.RULE15(po))
                        then po2es(po)
                        else ((bottom):EventSpaceErr)
                        fi .

    eq eventspace-ext(po, x) = if (po =/= ((bottom):PartialOrderErr<Action>)
				   and
				     rule-ext.RULE1(po, x)
				   and rule-ext.RULE2(po, x) and
				     rule-ext.RULE3(po, x) and
				       rule-ext.RULE4(po, x) and
                                   rule-ext.RULE5(po, x)
				   and rule-ext.RULE6(po, x) and
                                   rule-ext.RULE7(po, x) and
				     rule-ext.RULE8(po, x) and
                                   rule-ext.RULE9(po, x) and
				     rule-ext.RULE10(po, x) and
                                   rule-ext.RULE11(po, x) and
				     rule-ext.RULE12(po, x) and
                                   rule-ext.RULE13(po, x) and
				     rule-ext.RULE14(po, x) and
                                   rule-ext.RULE15(po, x))
                               then po2es(po)
                               else ((bottom):EventSpaceErr)
                               fi .
