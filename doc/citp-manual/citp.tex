...
本書は CITP for CafeOBJ(以降では\textsl{citp-cafe} と略称する)と呼ばれる%
証明譜作成支援システムの利用ガイドである．
読者が CafeOBJ 言語および形式的証明について基礎的な知識を持っていることを仮定している．

本書で記述する cit-cafe の機能は version 1.5.4 以降のCafeOBJ システムで利用可能である\footnote{最新版の CafeOBJ システムは http://cafeobj.org/download/ からダウンロード可能である}．

表\ref{table:terms}に本文内で使用するいくつかの用語の定義を示す．
...
用語 & 定義 \\\hline\hline
文(sentence)   & 自由変数を含まない(条件付き)等式($t = t' if C$)．
CafeOBJ の等式宣言の形式で表記する(CafeOBJの等式宣言では%
そこに含まれる変数が，暗黙的に全称記号($\forall$)によって束縛されていると見なされる事に注意．) \\\hline 
文脈(context) & citp-cafeでの証明は，ある CafeOBJ モジュール内で宣言された公理(等式)の集合から%
証明対象とする文を導出できることを示す事によって実施される．
このモジュールのことを(証明の)文脈と呼ぶ．
文脈は CafeOBJ インタプリタのコマンド ``select'' や ``open'' によって設定される．
\\\hline
...
本章では CITP for CafeOBJ で提供されるコマンドの機能と挙動について説明する．
説明にあたってはできるだけ例を示すように心がけた．
厳密な理解のためには参考文献\cite{Futatsugi:2012:PPS:2397725.2397950}を参照されたい．
...
\item 証明を開始するには，証明を行う文脈を設定し，次いで証明したい文の集合を宣言する．\\
  ...
  によって行う\footnote{<ModuleExpression> は "モジュール式'' と呼ばれ，%
あるモジュールから別のモジュールを系統的に定義するための式である．モジュール名もモジュール式の一つであり，そのモジュール自身を示す．}
\item 証明を実施する文脈の設定後，下に示す \verb|:goal| コマンドによって証明したい文の集合を指定する．\\
  ...
  \end{vvtm}
\end{itemize}
\verb|sentence| は，CafeOBJ の(条件付き)等式で表記する\footnote{
（条件付き)遷移規則を記述することも可能だが，CITP による証明は主として等式論理における%
証明を対象としたものであり，遷移規則の証明のためにはあまり馴染まない．}．

\subsubsection{文脈とゴールの設定例}
下は文脈の設定に引き続いたゴールの宣言の例である．

%\begin{vvtm}
  \begin{examplev}
  select LIST@ .
  :goal {eq [@ri]: (L1:List @ nil) = L1 .}
  \end{examplev}
%\end{vvtm}

ここでは事前に定義されている \texttt{LIST@} というモジュールを%
\texttt{select} コマンドにより文脈モジュールとして設定している．
その状態で \texttt{:goal} コマンドによって1個の文が証明対象の文として宣言されている．

\section{証明木の構造}\label{sec:proof-struct}
他のコマンドを説明する前に，CITP for CafeOBJ の構成する%
...
\item induction variables は \verb|:ind on| あるいは \verb|:ind {...}| コマンドによって指定する．
...
生成するが，
\verb|:ind{...}| は帰納法のベース，仮定，ステップのパターンを，
それぞれ \verb|base|，\verb|hypo|，\verb|step| で指定するものである．
