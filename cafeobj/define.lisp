...
(defstruct (oldoc (:print-function print-online-document))
  ...
  (doc-rtitle ""  :type string)         ; reduced title for search
...
(defun print-online-document (doc &optional (stream *standard-output*) &rest ignore)
  ...
  (format stream "~&red-title  : ~a" (oldoc-doc-rtitle doc))
...
(defun find-doc-entry (question)
  (let* ((redss (oldoc-reduce-string (oldoc-question-to-string question)))
	...
	(maphash #'(lambda (k v)
		     (if (and (>= (length (oldoc-doc-rtitle v)) redsslen)
			      (string-equal redss (subseq (oldoc-doc-rtitle v) 0 redsslen)))
			 (push (cons k (list (oldoc-doc-title v))) similar-keys)
...
(defun get-document-string-from-doc (doc &optional (raw nil))
  (if (not raw)
      (or (oldoc-cache doc)
...
(defun get-document-string (question &optional (raw nil))
  (let ((doc (find-doc-entry question)))
    (if (not (listp doc))
	(get-document-string-from-doc doc raw)
      (progn 
	(if (and doc (not raw))
	    (format t "Did you mean one of ~a~%" doc))
	nil))))

(defun get-example-string-from-doc (doc &optional (raw nil))
...
(defun get-example-string (key &optional (raw nil))
  (let ((doc (find-doc-entry key)))
    (if (not (listp doc))
	(get-example-string-from-doc doc raw)
      (progn
	(if (and doc (not raw))
	    (format t "Did you mean one of ~a~%" doc))
	nil))))
    ...
    (if re (let ((re (cl-ppcre:create-scanner re :case-insensitive-mode :multi-line-mode))
	     ...
	     (maphash #'(lambda (key oldoc)
			  ...
			  (let* ((fullss (format nil "~a~%~a~%~{~a~^~%~}~a~%~a"
						 (oldoc-doc-title oldoc)
						 (oldoc-doc-rtitle oldoc)
						 (oldoc-names oldoc)
						 (oldoc-doc-string oldoc)
						 (oldoc-doc-ex oldoc)))
...
(defun register-online-help (mainname aliasnames title mdkey doc example)
  ...
  (let ((mainkey (make-oldoc-key mainname)))
    (dolist (name (cons mainname aliasnames))
      (let ((key (make-oldoc-key name)))
  ...
  (let (data)
    (with-open-file (out output :direction :output :if-exists :supersede :if-does-not-exist :create)
      (maphash #'(lambda (k oldoc) 
		   (let ((docstr (get-document-string-from-doc oldoc t))
			 (exstr (get-example-string-from-doc oldoc t)))
		     (unless docstr
		       (error "The document string not found for ~s." k))
		     (if exstr (setq docstr (concatenate 'string docstr exstr)))
		     ...
		     ; (push (cons (oldoc-doc-rtitle oldoc) docstr) data)))
